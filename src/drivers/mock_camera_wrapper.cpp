...
struct MockCameraImpl {
    ...
    bool configure(const CameraConfig& config) {
        ...
        std::cout << "MockCamera: configured " << width << "x" << height
                  << " @ " << fps << " FPS (test pattern mode)" << std::endl;
        return true;
    }
    ...
    void generate_frame(cv::Mat& frame, uint64_t frame_id) {
        ...
        cv::Rect target_rect(
            static_cast<int>(target_pos.x - target_size),
        ...
        cv::line(frame, cv::Point(width/2 - 20, height/2),
                 cv::Point(width/2 + 20, height/2),
                 cv::Scalar(0, 255, 0), 2);
        cv::line(frame, cv::Point(width/2, height/2 - 20),
                 cv::Point(width/2, height/2 + 20),
                 cv::Scalar(0, 255, 0), 2);
        ...
        cv::putText(frame, text, cv::Point(10, 90), cv::FONT_HERSHEY_SIMPLEX,
                    0.6, cv::Scalar(255, 200, 0), 2);
    }

    void capture_loop() {
        const uint32_t frame_period_us = 1000000 / fps;
        ...
        while (running.load(std::memory_order_acquire)) {
            ...
            if (elapsed < frame_period_us) {
                std::this_thread::sleep_for(
                    std::chrono::microseconds(frame_period_us - elapsed));
            }
        }
    }
};
...
bool CameraWrapper::init() {
    return impl_->mock.configure(config_);
}
...
bool CameraWrapper::capture_frame(ZeroCopyFrame& frame, int /*timeout_ms*/) {
    ...
    frame.error[0] = 1;
    std::snprintf(frame.error + 1, sizeof(frame.error) - 1, "%s", "Mock frame - delete[] on cleanup");
