...
ImuReceiver::ImuReceiver(const ImuReceiverConfig& config)
    : config_(config),
      latest_data_{} {
...
bool ImuReceiver::init() {
    ...
    std::cout << "ImuReceiver: Listening on " << config_.bind_address << ":" 
              << config_.udp_port << "\n";
...
void ImuReceiver::parse_json_message(const std::string& json, ImuSample& sample) {
    ...
    // {"type": "android.sensor.accelerometer", "timestamp": 3925657519043709, "values": [0.32, -0.98, 10.05]}
