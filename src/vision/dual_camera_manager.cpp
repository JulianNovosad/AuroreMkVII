...
DualCameraManager::DualCameraManager(const DualCameraConfig& config)
    : config_(config) {
}
...
bool DualCameraManager::init_usb(const UsbCameraConfig& usb_config) {
    ...
    std::cout << "[DualCamera] USB camera initialized: " 
              << usb_config.width << "x" << usb_config.height << "@" 
              << usb_config.fps << "fps\n";
    return true;
}
...
std::optional<Detection> DualCameraManager::process_usb_frame() {
    ...
    if (orb_detector_ && orb_detector_->is_ready()) {
        ...
        if (detection.has_value()) {
            ...
            std::cout << "[DualCamera] USB detection: ROI(" 
                      << latest_roi_->x << "," << latest_roi_->y 
                      << "," << latest_roi_->w << "," << latest_roi_->h << ")\n";
        }
        return detection;
    }
