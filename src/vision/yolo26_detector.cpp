...
// ---------------------------------------------------------------------------
Yolo26Detector::Yolo26Detector(const Config& cfg)
...
bool Yolo26Detector::load() {
    ...
    try {
        ...
        for (auto& s : impl_->input_names_str)
            impl_->input_names.push_back(s.c_str());
        for (auto& s : impl_->output_names_str)
            impl_->output_names.push_back(s.c_str());

        loaded_ = true;
        std::cout << "[Yolo26] Model loaded: " << cfg_.model_path
...
// ---------------------------------------------------------------------------
Detection Yolo26Detector::unscale(int x1, int y1, int x2, int y2,
                                   float conf, int cls,
    ...
    const float rx2 = std::min(static_cast<float>(orig_w - 1),
                               (static_cast<float>(x2) - lb.pad_left) / lb.scale);
    const float ry2 = std::min(static_cast<float>(orig_h - 1),
                               (static_cast<float>(y2) - lb.pad_top)  / lb.scale);
...
std::vector<Detection> Yolo26Detector::detect_all(const cv::Mat& bgr_frame) {
    ...
    const size_t input_elements = 1 * 3 * static_cast<size_t>(cfg_.input_size) *
                                       static_cast<size_t>(cfg_.input_size);

    const float* data_ptr = reinterpret_cast<const float*>(lb.image.data);
    auto input_tensor = Ort::Value::CreateTensor<float>(
        impl_->mem_info, const_cast<float*>(data_ptr), input_elements,
    ...
    for (int64_t i = 0; i < num_det; ++i) {
        ...
        for (int tc : cfg_.target_classes) {
            if (tc == cls) { is_target = true; break; }
        }
