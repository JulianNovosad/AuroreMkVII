...
void test_rk4_vacuum_trajectory() {
    ...
    auto trajectory = solver.simulate_trajectory(muzzle_velocity, launch_angle_rad, air_density,
                                                 cross_section, mass, max_distance, dt);
    ...
    for (size_t i = 0; i < trajectory.size() && trajectory[i].z >= 0.f; ++i) {
        ...
        float vel_tolerance =
            0.01f * std::max(std::abs(expected_vx), std::abs(expected_vz)) + 0.01f;
    ...
    float expected_range =
        muzzle_velocity * muzzle_velocity * std::sin(2.f * launch_angle_rad) / 9.81f;
    ...
    std::cout << "PASS: RK4 vacuum trajectory matches analytical (range error: " << range_error
              << "m, expected: " << expected_range << "m)\n";
}
...
void test_g1_drag_drop() {
    ...
    std::cout << "  Impact horizontal velocity: " << impact_vx
              << " m/s (initial: " << muzzle_velocity << " m/s)\n";
}
