...
const char* state_to_string(TestState s) {
    switch (s) {
        case TestState::BOOT: return "BOOT";
        case TestState::IDLE_SAFE: return "IDLE_SAFE";
        case TestState::FREECAM: return "FREECAM";
...
const char* source_to_string(EstopSource s) {
    switch (s) {
        case EstopSource::HARDWARE: return "HARDWARE";
        case EstopSource::SOFTWARE: return "SOFTWARE";
        case EstopSource::WATCHDOG: return "WATCHDOG";
        case EstopSource::INTERLOCK: return "INTERLOCK";
        default: return "UNKNOWN";
    }
}
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    ...
    for (uint8_t s = 0; s <= 5; ++s) {
        for (uint8_t src = 0; src <= 3; ++src) {
            test_cases.push_back({static_cast<TestState>(s), 
                                static_cast<EstopSource>(src), 
                                false});
            test_cases.push_back({static_cast<TestState>(s), 
                                static_cast<EstopSource>(src), 
                                true});
