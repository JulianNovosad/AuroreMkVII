...
enum class TestState : uint8_t { SEARCH = 0, TRACKING = 1, ARMED = 2, FAULT_RECOVERY = 3 };
...
void run_watchdog_test(const TestParams& params) {
    ...
    if (!fault_triggered) {
        ...
        std::cerr << "FAIL: watchdog_miss_" << params.miss_count << "_duration_"
                  << params.miss_duration_ms << "ms"
                  << "_state_" << state_to_string(params.state) << "_period_"
                  << params.watchdog_period_ms << "ms" << std::endl;
        monitor.stop();
        return;
    }
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    ...
    const TestState states[] = {TestState::SEARCH, TestState::TRACKING, TestState::ARMED,
                                TestState::FAULT_RECOVERY};
