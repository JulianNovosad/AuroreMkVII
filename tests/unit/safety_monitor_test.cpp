...
TEST(test_safety_monitor_fault_clear) {
    ...
    // Verify clear_fault works when no emergency
...
TEST(test_fault_code_to_string) {
    ...
    ASSERT_TRUE(aurore::fault_code_to_string(aurore::SafetyFaultCode::VISION_LATENCY_EXCEEDED) != nullptr);
    ASSERT_TRUE(aurore::fault_code_to_string(aurore::SafetyFaultCode::ACTUATION_STALLED) != nullptr);
    ASSERT_TRUE(aurore::fault_code_to_string(aurore::SafetyFaultCode::EMERGENCY_STOP_REQUESTED) != nullptr);
    
    // Verify specific strings
    ASSERT_EQ(std::string(aurore::fault_code_to_string(aurore::SafetyFaultCode::NONE)), "NONE");
    ASSERT_EQ(std::string(aurore::fault_code_to_string(aurore::SafetyFaultCode::EMERGENCY_STOP_REQUESTED)), 
