...
static void test_security_multithreaded_race() {
    ...
    aurore::security::compute_hmac_sha256_raw_threadsafe(key, message.data(), message.size(),
                                                         expected_hmac);
    ...
    for (int t = 0; t < num_threads; t++) {
        threads.emplace_back([&, t]() {
            for (int i = 0; i < iterations_per_thread; i++) {
                ...
                aurore::security::compute_hmac_sha256_raw_threadsafe(key, message.data(),
                                                                     message.size(), current_hmac);
