...
inline const char* fault_code_to_string(SafetyFaultCode code) noexcept {
    switch (code) {
        case SafetyFaultCode::NONE: return "NONE";
        case SafetyFaultCode::VISION_STALLED: return "VISION_STALLED";
        case SafetyFaultCode::VISION_LATENCY_EXCEEDED: return "VISION_LATENCY_EXCEEDED";
        case SafetyFaultCode::VISION_BUFFER_OVERRUN: return "VISION_BUFFER_OVERRUN";
        case SafetyFaultCode::ACTUATION_STALLED: return "ACTUATION_STALLED";
        case SafetyFaultCode::ACTUATION_LATENCY_EXCEEDED: return "ACTUATION_LATENCY_EXCEEDED";
        case SafetyFaultCode::ACTUATION_COMMAND_INVALID: return "ACTUATION_COMMAND_INVALID";
        case SafetyFaultCode::FRAME_DEADLINE_MISSED: return "FRAME_DEADLINE_MISSED";
        case SafetyFaultCode::CONSECUTIVE_DEADLINE_MISSES: return "CONSECUTIVE_DEADLINE_MISSES";
        case SafetyFaultCode::TIMESTAMP_NON_MONOTONIC: return "TIMESTAMP_NON_MONOTONIC";
        case SafetyFaultCode::WATCHDOG_FEED_FAILED: return "WATCHDOG_FEED_FAILED";
        case SafetyFaultCode::MEMORY_LOCK_FAILED: return "MEMORY_LOCK_FAILED";
        case SafetyFaultCode::SCHEDULING_POLICY_FAILED: return "SCHEDULING_POLICY_FAILED";
        case SafetyFaultCode::I2C_TIMEOUT: return "I2C_TIMEOUT";
        case SafetyFaultCode::I2C_NACK: return "I2C_NACK";
        case SafetyFaultCode::CAMERA_TIMEOUT: return "CAMERA_TIMEOUT";
        case SafetyFaultCode::SAFETY_COMPARATOR_MISMATCH: return "SAFETY_COMPARATOR_MISMATCH";
        case SafetyFaultCode::INTERLOCK_FAULT: return "INTERLOCK_FAULT";
        case SafetyFaultCode::RANGE_DATA_STALE: return "RANGE_DATA_STALE";
        case SafetyFaultCode::RANGE_DATA_INVALID: return "RANGE_DATA_INVALID";
        case SafetyFaultCode::EMERGENCY_STOP_REQUESTED: return "EMERGENCY_STOP_REQUESTED";
        case SafetyFaultCode::CRITICAL_TEMPERATURE: return "CRITICAL_TEMPERATURE";
        case SafetyFaultCode::POWER_FAULT: return "POWER_FAULT";
...
struct SafetyEvent {
    ...
    SafetyEvent() noexcept
        : timestamp_ns(0)
        , fault_code(SafetyFaultCode::NONE)
        , severity(0) {
...
struct SafetyMonitorConfig {
    /// Vision pipeline max latency (default: 20ms per AM7-L2-VIS-003)
    uint64_t vision_deadline_ns = 20000000;
...
public:
    ...
    bool run_cycle() noexcept {
        ...
        if (consecutive_misses_.load(std::memory_order_acquire) >=
    ...
    /**
     * @brief Clear fault (after manual reset)
     * 
     * @return true if fault cleared, false if emergency active
     */
    bool clear_fault() noexcept {
        if (emergency_active_.load(std::memory_order_acquire)) {
            return false;  // Must power cycle or manual reset
        }
        
        system_safe_.store(true, std::memory_order_release);
        current_fault_.store(SafetyFaultCode::NONE, std::memory_order_release);
        consecutive_misses_.store(0, std::memory_order_release);
        deadline_misses_.store(0, std::memory_order_release);
...
private:
    void check_vision_health(TimestampNs now) noexcept {
        ...
        if (current_count == last_count && current_count > 0) {
            ...
            if (stall_duration > static_cast<int64_t>(config_.vision_deadline_ns * 2)) {
                trigger_fault(SafetyFaultCode::VISION_STALLED,
                             "Vision pipeline stalled", 3);
            }
        }
        ...
        if (last_ts > 0) {
            ...
            if (latency > static_cast<int64_t>(config_.vision_deadline_ns)) {
                ...
                snprintf(reason, sizeof(reason),
                        "Vision latency %ld ns exceeds %lu ns",
    ...
    void check_actuation_health(TimestampNs now) noexcept {
        ...
        if (current_count == last_count && current_count > 0) {
            ...
            if (stall_duration > static_cast<int64_t>(config_.actuation_deadline_ns * 2)) {
                trigger_fault(SafetyFaultCode::ACTUATION_STALLED,
                             "Actuation output stalled", 3);
            }
        }
        ...
        if (last_ts > 0) {
            ...
            if (latency > static_cast<int64_t>(config_.actuation_deadline_ns)) {
                char reason[MAX_FAULT_REASON_LEN];
                snprintf(reason, sizeof(reason),
                        "Actuation latency %ld ns exceeds %lu ns",
    ...
    void trigger_fault(SafetyFaultCode code, const char* reason, uint8_t severity) noexcept {
        ...
        while (!current_fault_.compare_exchange_weak(expected_fault, code,
        ...
        if (severity >= 3 || code == SafetyFaultCode::EMERGENCY_STOP_REQUESTED) {
            bool expected = false;
            if (emergency_active_.compare_exchange_strong(expected, true,
                    std::memory_order_acq_rel, std::memory_order_acquire)) {
    ...
    void watchdog_thread_func() noexcept {
        while (watchdog_running_.load(std::memory_order_acquire)) {
            ...
            const uint64_t elapsed_ms = elapsed_ns > 0 ? static_cast<uint64_t>(elapsed_ns) / 1000000 : 0;

            // Check if watchdog timeout exceeded
            if (elapsed_ms > config_.watchdog_timeout_ms) {
                trigger_fault(
                    SafetyFaultCode::WATCHDOG_FEED_FAILED,
                    "Software watchdog timeout - no kick received",
                    4
                );
            }
...
public:
     ...
     */
    explicit WatchdogKick(SafetyMonitor& monitor) noexcept
        : monitor_(monitor) {}

    /**
     * @brief Destructor (kicks watchdog)
     */
    ~WatchdogKick() noexcept {
        monitor_.kick_watchdog();
    }
