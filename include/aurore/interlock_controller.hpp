...
inline const char* interlock_state_to_string(InterlockState state) noexcept {
    switch (state) {
        case InterlockState::OPEN: return "OPEN";
        case InterlockState::CLOSED: return "CLOSED";
        case InterlockState::FAULT: return "FAULT";
        case InterlockState::UNKNOWN: return "UNKNOWN";
...
struct InterlockConfig {
    ...
    /// GPIO pin for inhibit output (BCM numbering)
    int inhibit_pin = 27;
    ...
    bool validate() const noexcept {
        ...
        if (inhibit_pin < 0 || inhibit_pin > 27) return false;
        if (status_led_pin < 0 || status_led_pin > 27) return false;
        
        // Pins must be unique
        if (input_pin == inhibit_pin || input_pin == status_led_pin || 
            inhibit_pin == status_led_pin) return false;
 ...
 * @brief GPIO-based safety interlock controller
 *
 * Monitors hardware interlock circuit and controls inhibit output.
 ...
 *     
 *     InterlockController interlock(config);
 *     interlock.init();
 *     interlock.start();
 *     
 *     // In safety monitor thread:
...
public:
     ...
     */
    explicit InterlockController(const InterlockConfig& config = InterlockConfig());
     ...
     */
    InterlockState get_state() const noexcept {
     ...
     */
    int get_input_raw() const noexcept {
        return input_value_.load(std::memory_order_acquire);
    }
     ...
     */
    int get_output_raw() const noexcept {
        return output_value_.load(std::memory_order_acquire);
    }
