 ...
 * Aligned to 64 bytes for SIMD optimization (AVX-512, ARM NEON).
 *
 * Security: All pointer fields must be validated before dereferencing.
 * Use validate() to check frame integrity.
 *
 * Authentication (per ICD-001 and AM7-L2-SEC-001):
 * - frame_hash: SHA256 hash of pixel data (computed asynchronously)
 * - hmac: HMAC-SHA256 over header + frame_hash (256-bit key from config)
 */
struct alignas(64) ZeroCopyFrame {
    ...
    uint8_t frame_hash[32];  ///< SHA256 hash of pixel data (ICD-001 authentication)
    uint8_t hmac[32];        ///< HMAC-SHA256 over header + frame_hash (AM7-L2-SEC-001)
    ...
    /**
     * @brief Check if frame authentication is present (non-zero hash/hmac)
     *
     * @return true if frame_hash and hmac are non-zero
     */
    bool has_authentication() const noexcept {
        // Quick check: if first byte of hash or hmac is non-zero, assume authenticated
        return (frame_hash[0] != 0 || hmac[0] != 0);
    }

    /**
     * @brief Verify HMAC authentication
     *
     * @param key HMAC key (256-bit recommended)
     * @param key_len Length of key in bytes
     * @return true if HMAC verification passes
     */
    bool verify_authentication(const void* key, size_t key_len) const noexcept;
};

// Compile-time check: ZeroCopyFrame must be trivially copyable for lock-free ring buffer
static_assert(std::is_trivially_copyable_v<ZeroCopyFrame>,
              "ZeroCopyFrame must be trivially copyable");
...
class CameraWrapper {
   public:
    ...
    /**
     * @brief Release frame (must be called when done with zero-copy buffer)
     *
     ...
     */
    uint64_t error_count() const noexcept { return error_count_.load(std::memory_order_acquire); }
...
class FrameBufferAllocator {
   public:
     ...
     */
    size_t get_size(int plane = 0) const noexcept { return plane_size_[plane]; }
     ...
     */
    int get_stride(int plane = 0) const noexcept { return stride_[plane]; }
