...
inline void DriverOptions::UnPackTo(DriverOptionsT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  ...
  { auto _e = usb(); if (_e) { if(_o->usb) { _e->UnPackTo(_o->usb.get(), _resolver); } else { _o->usb = std::unique_ptr<platforms::darwinn::api::DriverUsbOptionsT>(_e->UnPack(_resolver)); } } else if (_o->usb) { _o->usb.reset(); } }
